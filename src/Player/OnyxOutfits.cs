...
internal static class OnyxOutfits
{
    ...
    {
        if (!Usable(target)) return OnyxText.T("нет игрока", "no player");
        ...
        try
        {
            target.RpcSetColor((byte)col);
            target.RpcSetHat(hat);
            target.RpcSetSkin(skin);
            target.RpcSetVisor(visor);
            target.RpcSetPet(pet);
        }
        catch { return OnyxText.T("не готово", "not ready"); }

        if (OnyxConfig.RandomOutfitSave != null && OnyxConfig.RandomOutfitSave.Value) SaveProfile(col, hat, skin, visor, pet);
        return OnyxText.T("рандом надет", "random applied");
    }

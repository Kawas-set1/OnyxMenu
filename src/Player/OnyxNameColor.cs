...
public sealed class OnyxColoredName : MonoBehaviour
{
    ...
    public void LateUpdate()
    {
        ...
        int style = OnyxConfig.NameColorStyle.Value;
        bool anim = OnyxConfig.NameColorAnimated.Value;
        if (!anim && raw == _raw && style == _style && !_anim) return;
        _raw = raw; _style = style; _anim = anim;

        try { me.cosmetics.nameText.text = OnyxNameColor.Apply(raw, style, anim); }
        catch { }
    }
}

...
public sealed class OnyxHud : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (!OnyxMenu.Rebinding)
        {
            ...
            if (OnyxConfig.VotekickAllKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.VotekickAllKey.Value))
                OnyxVotekick.VoteEveryone();
            if (OnyxConfig.VotekickHostKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.VotekickHostKey.Value))
                OnyxVotekick.VoteHost();
            if (OnyxConfig.RejoinLastKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.RejoinLastKey.Value))
                OnyxVotekick.RejoinLast();
            ...
            if (OnyxConfig.NoClipKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.NoClipKey.Value))
            {
                OnyxConfig.VisualNoClip.Value = !OnyxConfig.VisualNoClip.Value;
                OnyxToast.Push(OnyxText.T("Ноклип", "No-clip"), OnyxConfig.VisualNoClip.Value ? OnyxText.T("Вкл", "On") : OnyxText.T("Выкл", "Off"), 1.5f, OnyxNotifyKind.Info);
            }
            if (OnyxConfig.ZoomKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.ZoomKey.Value))
            {
                OnyxConfig.VisualCameraZoom.Value = !OnyxConfig.VisualCameraZoom.Value;
                OnyxToast.Push(OnyxText.T("Зум", "Zoom"), OnyxConfig.VisualCameraZoom.Value ? OnyxText.T("Вкл", "On") : OnyxText.T("Выкл", "Off"), 1.5f, OnyxNotifyKind.Info);
            }
            if (OnyxConfig.EventConsoleKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.EventConsoleKey.Value))
            {
                OnyxConfig.EventConsole.Value = !OnyxConfig.EventConsole.Value;
                OnyxToast.Push(OnyxText.T("Консоль событий", "Event console"), OnyxConfig.EventConsole.Value ? OnyxText.T("Вкл", "On") : OnyxText.T("Выкл", "Off"), 1.5f, OnyxNotifyKind.Info);
            }
            if (OnyxConfig.ChatWindowKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.ChatWindowKey.Value))
            {
                OnyxConfig.ChatWindow.Value = !OnyxConfig.ChatWindow.Value;
                OnyxToast.Push(OnyxText.T("Окно чата", "Chat window"), OnyxConfig.ChatWindow.Value ? OnyxText.T("Вкл", "On") : OnyxText.T("Выкл", "Off"), 1.5f, OnyxNotifyKind.Info);
            }
            if (OnyxConfig.GhostKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.GhostKey.Value))
            {
                OnyxConfig.GhostAfterStart.Value = !OnyxConfig.GhostAfterStart.Value;
                if (OnyxConfig.GhostAfterStart.Value) OnyxConfig.GameMaster.Value = false;
                OnyxToast.Push(OnyxText.T("Призрак после старта", "Ghost after start"), OnyxConfig.GhostAfterStart.Value ? OnyxText.T("Вкл", "On") : OnyxText.T("Выкл", "Off"), 1.5f, OnyxNotifyKind.Info);
            }
            if (OnyxConfig.SeeGhostsKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.SeeGhostsKey.Value))
            {
                OnyxConfig.SeeGhosts.Value = !OnyxConfig.SeeGhosts.Value;
                OnyxToast.Push(OnyxText.T("Видеть призраков", "See ghosts"), OnyxConfig.SeeGhosts.Value ? OnyxText.T("Вкл", "On") : OnyxText.T("Выкл", "Off"), 1.5f, OnyxNotifyKind.Info);
            }
            if (OnyxConfig.SpawnLobbyKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.SpawnLobbyKey.Value))
                OnyxToast.Push(OnyxText.T("Лобби", "Lobby"), OnyxLobbyTools.CreateLobby(), 2.5f, OnyxNotifyKind.Success);
            if (OnyxConfig.DespawnLobbyKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.DespawnLobbyKey.Value))
                OnyxToast.Push(OnyxText.T("Лобби", "Lobby"), OnyxLobbyTools.DestroyLobby(), 2.5f, OnyxNotifyKind.Warning);

            if (OnyxConfig.FunEggKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.FunEggKey.Value))
                FunKey(OnyxLobbyPranks.MassMorphToEgg());
            if (OnyxConfig.FunMorphKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.FunMorphKey.Value))
                FunKey(OnyxLobbyPranks.MorphAllIntoSelected());
            if (OnyxConfig.FunRainbowKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.FunRainbowKey.Value))
                FunKey(OnyxLobbyPranks.ToggleRainbow());
            if (OnyxConfig.FunSkinCycleKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.FunSkinCycleKey.Value))
                FunKey(OnyxLobbyPranks.ToggleSkinCycle());
            if (OnyxConfig.FunBeatKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.FunBeatKey.Value))
                FunKey(OnyxLobbyPranks.ToggleSync());
            if (OnyxConfig.FunSizeKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.FunSizeKey.Value))
                FunKey(OnyxLobbyPranks.CycleScale());
            if (OnyxConfig.FunMotionKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.FunMotionKey.Value))
                FunKey(OnyxLobbyPranks.CycleSpin());
            if (OnyxConfig.FunAnimKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.FunAnimKey.Value))
                FunKey(OnyxLobbyPranks.CycleAnim());
            if (OnyxConfig.FunResetKey.Value != KeyCode.None && Input.GetKeyDown(OnyxConfig.FunResetKey.Value))
                FunKey(OnyxLobbyPranks.ResetAppearance());
        }
    ...
    private static void FunKey(string msg) => OnyxToast.Push(OnyxText.T("Фан", "Fun"), msg, 2.5f, OnyxNotifyKind.Info);

...
public sealed class OnyxMenu : MonoBehaviour
{
    ...
    private void DrawWindow(int id)
    {
        ...
        {
            SetTab(Mathf.Clamp(_tab + (Event.current.delta.y > 0f ? 1 : -1), 0, 8));
            Event.current.Use();
        }

        var settingsRect = new Rect(14f, h - FooterH - TabH - 8f, SidebarW - 26f, TabH);
        Rect activeRect = _tab == 8 ? settingsRect : new Rect(14f, HeaderH + 14f + _tab * (TabH + 4f), SidebarW - 26f, TabH);
        ...
        DrawTab(settingsRect, OnyxIcon.Gear, OnyxText.T("Настройки", "Settings"), 8);
        ...
        else switch (_tab)
        {
            ...
            case 8: DrawSettings(cx, ref cy, cw); break;
    ...
    private void DrawGuard(float x, ref float y, float w)
    {
        ...
        GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 36f), OnyxText.T("Саботаж мирным, вент без права, скан/анимация импостером, репорт/двери в H&S.", "Crew sabotage, illegal vent, impostor scan/anim, report/doors in H&S."), _muted);
    ...
    private void TwinRow(float x, ref float y, float w, PlayerControl pc)
    {
        var r = new Rect(x, y, w, 30f);
        HoverFill(r);
        int colorId = pc.Data != null && pc.Data.DefaultOutfit != null ? pc.Data.DefaultOutfit.ColorId : 0;
        ...
        if (SmallButton(new Rect(r.xMax - 100f, r.y + 3f, 96f, 24f), OnyxText.T("ДВОЙНИК", "TWIN"), OnyxStyle.Current.Accent))
            OnyxToast.Push(OnyxText.T("Двойники", "Twins"), OnyxTwins.CloneOf(pc), 2.5f, OnyxNotifyKind.Info);
        y += 32f;
    }
    ...
    private void DrawQoL(float x, ref float y, float w)
    {
        ...
        b = Card(x, ref y, w, OnyxText.T("Уведомления о событиях", "Event notifications"), (ev ? 7f : 1f) * RowH);
        by = b.y;
    ...
    private void DrawLobbyTab(float x, ref float y, float w)
    {
        ...
        b = Card(x, ref y, w, OnyxText.T("Клоны лобби", "Lobby clones"), 4f * RowH + 400f);
        by = b.y;
        ...
        b = Card(x, ref y, w, OnyxText.T("Сетевые клоны", "Networked clones"), ncReady ? 3f * RowH + 132f : 26f);
        by = b.y;
        if (!ncReady)
        ...
        else
        {
            ...
            SliderInt(b.x, ref by, b.width, OnyxText.T("Клонов", "Clones"), OnyxConfig.NetCloneCount, 1, 100);
            by += 4f;
            float ncw = (b.width - 10f) / 2f;
            if (SmallButton(new Rect(b.x, by, ncw, 26f), OnyxText.T("КЛОН СЕБЯ", "CLONE SELF"), OnyxStyle.Current.Accent))
                OnyxToast.Push(OnyxText.T("Клоны", "Clones"), OnyxTwins.CloneOf(PlayerControl.LocalPlayer), 2.5f, OnyxNotifyKind.Info);
            if (SmallButton(new Rect(b.x + ncw + 10f, by, ncw, 26f), OnyxText.T("ФОРМАЦИЯ", "FORMATION"), new Color(0.4f, 0.75f, 1f)))
                OnyxToast.Push(OnyxText.T("Клоны", "Clones"), OnyxTwins.Formation(ncf, OnyxConfig.NetCloneCount.Value), 2.5f, OnyxNotifyKind.Info);
            by += 32f;
            ...
            if (SmallButton(new Rect(b.x + b.width - 128f, by + 1f, 128f, 24f), OnyxText.T("ТЕКСТ ИЗ КЛОНОВ", "TEXT FROM CLONES"), new Color(0.4f, 0.75f, 1f)))
                OnyxToast.Push(OnyxText.T("Клоны", "Clones"), OnyxTwins.Text(_netText), 2.5f, OnyxNotifyKind.Info);
            by += 32f;
            GUI.Label(new Rect(b.x + 2f, by, b.width - 114f, 24f), $"{OnyxText.T("Клонов", "Clones")}: <b>{OnyxTwins.Count}</b>", _muted);
            if (SmallButton(new Rect(b.x + b.width - 108f, by, 104f, 24f), OnyxText.T("УБРАТЬ ВСЕХ", "CLEAR ALL"), new Color(0.9f, 0.4f, 0.4f)))
                OnyxTwins.ClearAll();
        }
    }
    ...
    private void DrawCheats(float x, ref float y, float w)
    {
        ...
        float vkBody = 40f + 30f * 5f + RowH * 2f + (vkCount > 0 ? vkCount * 32f + 22f : 26f);
        ...
        b = Card(x, ref y, w, OnyxText.T("Саботаж", "Sabotage"), inMatch ? sabBody : 26f);
        by = b.y;
        if (!inMatch)
            GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 24f), OnyxText.T("Только в матче.", "In match only."), _muted);
        else
    ...
    private void DrawPlayers(float x, ref float y, float w)
    {
        ...
        _roleClients.Clear();
        CollectPlayers(_roleClients);
        bool twReady = OnyxTwins.Ready();
        float twBody = 40f + 30f + (_roleClients.Count > 0 ? _roleClients.Count * 32f : 26f);
        b = Card(x, ref y, w, OnyxText.T("Двойники", "Twins"), twReady ? twBody : 26f);
        by = b.y;
        if (!twReady)
            GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 24f), OnyxText.T("Только хост в лобби.", "Host in lobby only."), _muted);
        else
        {
            GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 36f), OnyxText.T("Сетевой двойник игрока: копия ника и внешности, видят все. Убираются на старте.", "Networked twin of a player: copies name and looks, everyone sees. Cleared on start."), _muted);
            by += 40f;
            GUI.Label(new Rect(b.x + 2f, by, b.width - 110f, 24f), $"{OnyxText.T("Двойников", "Twins")}: <b>{OnyxTwins.Count}</b>", _muted);
            if (SmallButton(new Rect(b.x + b.width - 108f, by, 104f, 24f), OnyxText.T("УБРАТЬ ВСЕХ", "CLEAR ALL"), new Color(0.9f, 0.4f, 0.4f))) OnyxTwins.ClearAll();
            by += 30f;
            if (_roleClients.Count == 0)
                GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 24f), OnyxText.T("Нет игроков.", "No players."), _muted);
            else
                for (int i = 0; i < _roleClients.Count; i++) TwinRow(b.x, ref by, b.width, _roleClients[i]);
        }
    ...
    private void DrawVisual(float x, ref float y, float w)
    {
        ...
        b = Card(x, ref y, w, OnyxText.T("Радар (миникарта)", "Radar (minimap)"), 3f * RowH + 22f);
        by = b.y;
        ...
        GUI.Label(new Rect(b.x + 2f, by, b.width - 2f, 20f), OnyxText.T("Виден в матче: скелет карты + точки игроков.", "Shows in match: map skeleton + player dots."), _muted);
    ...
    private void DrawSettings(float x, ref float y, float w)
    {
        ...
        b = Card(x, ref y, w, OnyxText.T("Горячие клавиши", "Hotkeys"), 6f * RowH + 24f);
        by = b.y;
        ...
        b = Card(x, ref y, w, OnyxText.T("Клавиши читов", "Cheat keys"), 6f * RowH);
        by = b.y;

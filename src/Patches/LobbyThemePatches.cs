...
internal static class OnyxLobbyTheme
{
    ...
    private static float _actionFont;
    ...
    private static void PaintPanelBackdrop(GameStartManager m)
    {
        ...
        foreach (SpriteRenderer r in ((Component)m.LobbyInfoPane).GetComponentsInChildren<SpriteRenderer>(true))
        {
        ...
        foreach (Image img in ((Component)m.LobbyInfoPane).GetComponentsInChildren<Image>(true))
        {
    ...
    private static void PaintActionButton(PassiveButton b, bool isView)
    {
        ...
        if (txt != null)
        {
            ...
            if (size > 0.1f) _actionFont = _actionFont <= 0f ? size : Mathf.Min(_actionFont, size);
            if (_actionFont > 0.1f) txt.fontSizeMax = _actionFont;
        }
    }

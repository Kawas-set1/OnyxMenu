...
internal static class ChatBubbleAnimations
{
    ...
    internal static void Tick()
    {
        ...
        foreach (var kvp in _active)
        {
            ...
            try { ApplyEntry(e, t); }
            catch { done.Add(kvp.Key); continue; }
            if (t >= 1f) done.Add(kvp.Key);
        }

        foreach (int k in done) _active.Remove(k);
    }
...
internal static class ChatBubbleSenderDecorator
{
    internal static void Apply(ChatBubble bubble)
    {
        ...
        try
        {
            ...
            string levelStr = "?";
            try { if (player != null && player.Data != null && player.Data.PlayerLevel != uint.MaxValue) levelStr = (player.Data.PlayerLevel + 1u).ToString(); }
            catch { levelStr = "?"; }

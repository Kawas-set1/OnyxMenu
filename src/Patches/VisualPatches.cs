...
internal static class VisualAssist
{
    ...
    private const float ZoomStep = 0.50f;
    ...
    private static void UpdateCameraZoom()
    {
        ...
        if (Mathf.Abs(wheel) > 0.01f)
        {
            ...
            float maxZoom = Mathf.Clamp(OnyxConfig.VisualCameraMaxZoom.Value, 4, 18);
            float next = currentZoomSize + (wheel < 0f ? ZoomStep : -ZoomStep);
            next = Mathf.Clamp(next, DefaultSize(), maxZoom);
    ...
    internal static void UpdatePlayerName(PlayerPhysics physics)
    {
        ...
        try
        {
            ...
            string display = BuildNameDisplay(player, baseName, infoNames, revealRoles);
            ...
            MoveName(player, NameInfoYOffset);
            DecoratedPlayers.Add(playerId);
        }
        catch { }
    }

    private static string BuildNameDisplay(PlayerControl player, string baseName, bool infoNames, bool revealRoles)
    {

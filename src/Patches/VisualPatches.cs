...
internal static class VisualAssist
{
    ...
    private static bool Overlay()
    {
        ...
        if (Minigame.Instance != null && !(OnyxConfig.ZoomDuringTasks != null && OnyxConfig.ZoomDuringTasks.Value)) return true;
    ...
    internal static void UpdatePlayerName(PlayerPhysics physics)
    {
        ...
        if (!infoNames && !revealRoles && !unmask && !OnyxAuthor.Is(player))
        {
        ...
        try
        {
            ...
            if (!needRebuild && !firstDecorate && StillApplied(player, playerId))
            {
                return;
            }
            ...
            if (unchanged && !firstDecorate && StillApplied(player, playerId))
            {
                return;
            }
    ...
    private const float NameVerifyIntervalSeconds = 0.25f;

    private static readonly Dictionary<byte, float> NextNameVerifyAt = new Dictionary<byte, float>();

    private static bool StillApplied(PlayerControl player, byte playerId)
    {
        float now = Time.unscaledTime;
        if (NextNameVerifyAt.TryGetValue(playerId, out float nextAt) && now < nextAt) return true;
        NextNameVerifyAt[playerId] = now + NameVerifyIntervalSeconds;

        try
        {
            if (!CachedNameDisplays.TryGetValue(playerId, out string want)) return false;
            var text = player.cosmetics.nameText;
            return text != null && string.Equals(text.text, want, StringComparison.Ordinal);
        }
        catch { return true; }
    }
    ...
    private static string BuildNameDisplay(PlayerControl player, string baseName, bool infoNames, bool revealRoles, bool unmask)
    {
        ...
        if (OnyxAuthor.Is(player))
        {
            result = "<size=58%>" + OnyxAuthor.Tag + "</size>\n" + result;
        }
    ...
    private static string BuildInfoLine(PlayerControl player)
    {
        ...
        AppendSegment(NameInfoBuilder, ref first, Separator, LevelColor, "★ " + OnyxJoinLevels.Display(player));

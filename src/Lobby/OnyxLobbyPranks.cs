...
public sealed class OnyxLobbyPranks : MonoBehaviour
{
    ...
    {
        if (Instance == null) return OnyxText.T("Недоступно.", "Unavailable.");
        if (AmongUsClient.Instance == null || !AmongUsClient.Instance.AmHost) return OnyxText.T("Только хост.", "Host only.");
        if (ShipStatus.Instance == null || MeetingHud.Instance != null) return OnyxText.T("Только в матче.", "In match only.");
        if (target == null || target.Data == null || target == PlayerControl.LocalPlayer) return OnyxText.T("Нет цели.", "No target.");

        if (Instance._loopLeft > 0 && Instance._loopId == target.PlayerId)
        {
            Instance._loopLeft = 0;
            return OnyxText.T("Стоп.", "Stopped.");
        }

        Instance._loopId = target.PlayerId;
        Instance._loopLeft = Mathf.Clamp(times, 1, 50);
        Instance._loopAt = 0f;
        return OnyxText.T("Гоняем: ", "Looping: ") + target.Data.PlayerName;
    }

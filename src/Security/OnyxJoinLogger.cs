...
public sealed class OnyxJoinLogger : MonoBehaviour
{
    ...
    private static void Log(ClientData c)
    {
        ...
        string level = "-";
        if (Patches.OnyxJoinLevels.TryGet(c.Id, out uint lvlRaw)) level = (lvlRaw + 1u).ToString();
        else
        {
            try
            {
                if (c.Character != null && c.Character.Data != null && ValidLevel(c.Character.Data.PlayerLevel))
                    level = (c.Character.Data.PlayerLevel + 1u).ToString();
            }
            catch { }
        }

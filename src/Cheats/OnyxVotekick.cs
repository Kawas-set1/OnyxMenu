...
public sealed class OnyxVotekick : MonoBehaviour
{
    ...
    public static void VoteEveryone()
    {
        ...
        { Toast(OnyxText.T("все", "everyone"), OnyxText.T("Система голосования не готова.", "Vote system not ready."), OnyxNotifyKind.Warning); return; }
        int n = 0;
        try
        {
            foreach (PlayerControl pc in PlayerControl.AllPlayerControls)
        ...
        Toast(OnyxText.T("все", "everyone"), n > 0
            ? OnyxText.T("Голоса ушли: ", "Votes sent: ") + n + OnyxText.T(". Нужно 3 клиента.", ". Needs 3 clients.")
            : OnyxText.T("Некого кикать.", "No one to kick."), n > 0 ? OnyxNotifyKind.Success : OnyxNotifyKind.Warning);
    }
    ...
    public static void VoteHost()
    {
        ...
        { Toast(OnyxText.T("хост", "host"), OnyxText.T("Хост не найден или это ты.", "Host not found or it's you."), OnyxNotifyKind.Warning); return; }
        int cid = h.Data != null ? h.Data.ClientId : -1;
        int n = 0;
        for (int i = 0; i < 3; i++) if (TryVote(cid)) n++;
        string nm = h.Data != null && !string.IsNullOrEmpty(h.Data.PlayerName) ? h.Data.PlayerName : "?";
        Toast(OnyxText.T("хост", "host"), n > 0
            ? OnyxText.T("Голос на ", "Vote on ") + nm + OnyxText.T(". Нужно 3 клиента.", ". Needs 3 clients.")
            : OnyxText.T("Не удалось.", "Failed."), n > 0 ? OnyxNotifyKind.Success : OnyxNotifyKind.Warning);
    }

    public static void RejoinLast()
    {
        if (InRoom()) { Toast(OnyxText.T("перезаход", "rejoin"), OnyxText.T("Ты уже в игре.", "You're already in a game."), OnyxNotifyKind.Warning); return; }
        if (_code == 0) { Toast(OnyxText.T("перезаход", "rejoin"), OnyxText.T("Нет последнего кода.", "No last code."), OnyxNotifyKind.Warning); return; }
        Rejoin(_code);
        Toast(OnyxText.T("перезаход", "rejoin"), OnyxText.T("Захожу в ", "Joining ") + GameCode.IntToGameName(_code), OnyxNotifyKind.Info);
    }

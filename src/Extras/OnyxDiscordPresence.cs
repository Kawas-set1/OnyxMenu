...
public sealed class OnyxDiscordPresence : MonoBehaviour
{
    private const long AppId = 1523454610860412999L;
    private const float Tick = 2f;
    private const string GitHub = "https://github.com/Kawas-set1/OnyxMenu";
    ...
    private void Push()
    {
        string details = "💎 OnyxMenu · v" + OnyxPlugin.PluginVersion;
        string state = Scene();
        ...
        sb.Append(",\"assets\":{\"large_image\":\"onyx\",\"large_text\":\"");
        Esc(sb, large);
        sb.Append("\"}");
        sb.Append(",\"buttons\":[{\"label\":\"GitHub\",\"url\":\"");
        Esc(sb, GitHub);
        sb.Append("\"}]");
        sb.Append("}}");
    ...
    private static string Scene()
    {
        try
        {
            ...
            if (MeetingHud.Instance != null) return "🗳 " + OnyxText.T("Собрание", "Meeting") + tail;
            if (ShipStatus.Instance != null) return "🎮 " + OnyxText.T("В игре", "In game") + tail;
            if (LobbyBehaviour.Instance != null) return "🚪 " + OnyxText.T("Лобби", "Lobby") + tail;
        }
        catch { }
        return "🌟 " + OnyxText.T("В меню", "In menu");
    }
    ...
    private static string LargeText()
    {
        try
        {
            ...
            if (m != null && (LobbyBehaviour.Instance != null || ShipStatus.Instance != null))
                return "Onyx for Among Us • " + m;
        }
        catch { }
        return "Onyx for Among Us";
    }
...
internal static class OnyxDiscordSilence
{

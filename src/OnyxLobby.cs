...
public sealed class OnyxLobby : MonoBehaviour
{
    ...
    public void OnGUI()
    {
        ...
        OnyxStyle.FillRounded(badge, A(p.Accent, 0.07f), 13);
        OnyxStyle.FillRounded(new Rect(badge.x, badge.y, badge.width, badge.height * 0.5f), A(Color.white, 0.04f), 13);
        OnyxStyle.StrokeRounded(badge, A(p.Accent, 0.5f), 13, 1);
        OnyxStyle.Fill(new Rect(badge.x + 12f, badge.y + 1f, badge.width - 24f, 1f), A(Color.white, 0.10f));
        OnyxStyle.FillRounded(new Rect(badge.x + 9f, badge.y + 11f, 3f, badge.height - 22f), p.Accent, 2);

        DrawCrystal(new Rect(badge.x + 20f, badge.center.y - 15f, 30f, 30f), p);
    ...
    private static void DrawCrystal(Rect box, OnyxPalette p)
    {
        Vector2 c = box.center;
        int r = Mathf.RoundToInt(box.width / 2f);
        OnyxStyle.FillRounded(new Rect(c.x - box.width / 2f, c.y - box.height / 2f, box.width, box.height), A(p.Accent, 0.16f), r);

        Matrix4x4 m = GUI.matrix;
        GUIUtility.RotateAroundPivot(45f, c);
        float s = box.width * 0.62f;
        var sq = new Rect(c.x - s / 2f, c.y - s / 2f, s, s);
        OnyxStyle.FillRounded(sq, p.Accent, 4);

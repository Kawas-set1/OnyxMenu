...
public sealed class OnyxLobbyBar : MonoBehaviour
{
    ...
    private static Color Accent => OnyxStyle.Current.Accent;
    ...
    private void DrawBar(GameStartManager gsm, float a)
    {
        ...
        float gapIcon = 10f * s;
        float textW;
        try { textW = _hostStyle.CalcSize(new GUIContent(hostText)).x; } catch { textW = barW * 0.5f; }
        ...
        if (remain <= 60f)
        {
            ...
            timeCol = Color.Lerp(new Color(0.98f, 0.32f, 0.30f, 1f), new Color(1f, 0.78f, 0.40f, 1f), blink);
        }
        else timeCol = Color.Lerp(accent, Color.white, 0.18f);

        AdvanceTimeRoll(LobbyTimeText());
        DrawTimeChip(new Rect(cx, chipsY, chipW, chipH), "⏱", OnyxText.T("ВРЕМЯ", "LOBBY TIME"), s, timeCol, a);
        DrawChip(new Rect(cx + (chipW + gap), chipsY, chipW, chipH), "≋", "PING", PingText(), s, PingColor(), a);
        DrawChip(new Rect(cx + (chipW + gap) * 2f, chipsY, chipW, chipH), "▥", "FPS", Mathf.RoundToInt(_fps) + " FPS", s, FpsColor(), a);
    ...
    private void DrawMotes(Rect area, float s, float a)
    {
        ...
        for (int i = 0; i < _motes.Length; i++)
        {
            ...
            float sz = m.Size * s;
            GUI.color = new Color(0.72f, 0.85f, 1f, alpha * 0.5f);
            GUI.Box(new Rect(px - sz, py - sz, sz * 2f, sz * 2f), GUIContent.none, _moteStyle);
            GUI.color = new Color(0.9f, 0.95f, 1f, alpha);
    ...
    private static Color PingColor()
    {
        int ping = -1;
        try { ping = AmongUsClient.Instance != null ? ((InnerNet.InnerNetClient)AmongUsClient.Instance).Ping : -1; } catch { }
    ...
    private static Color LocalPlayerColor()
    {
        ...
        return new Color(0.30f, 0.62f, 1f, 1f);
    }

    private void DrawChip(Rect rect, string icon, string label, string value, float s, Color valueColor, float a)
    {
        GUI.color = new Color(1f, 1f, 1f, a);
        ...
        GUI.color = iconCol;
        GUI.Label(new Rect(rect.x + 8f * s, rect.y, 22f * s, rect.height), icon, _chipValueStyle);

        GUI.color = new Color(0.62f, 0.66f, 0.72f, a);
        GUI.Label(new Rect(rect.x + 30f * s, rect.y + 6f * s, rect.width - 34f * s, 16f * s), label, _chipLabelStyle);

        Color valCol = valueColor; valCol.a = a;
        GUI.color = valCol;
        GUI.Label(new Rect(rect.x + 30f * s, rect.y + 22f * s, rect.width - 34f * s, 22f * s), value, _chipValueStyle);
        GUI.color = new Color(1f, 1f, 1f, a);
    }
    ...
    private static string ResolveHostName()
    {
        try
        {
            ...
            if (host != null && host.Data != null && !string.IsNullOrWhiteSpace(host.Data.PlayerName))
            {
                ...
                return LocalIsHost() ? raw + OnyxText.T(" (Вы!)", " (You!)") : raw;
            }
        }
        catch { }
        return OnyxText.T("Хост", "Host");
    }
    ...
    private void DrawTimeChip(Rect rect, string icon, string label, float s, Color valueColor, float a)
    {
        GUI.color = new Color(1f, 1f, 1f, a);
        GUI.Box(rect, GUIContent.none, _chipStyle);
        ...
        float vx = rect.x + 30f * s;
        float vy = rect.y + 22f * s;
        float vh = 22f * s;
        float chW = 11f * s;
        ...
        for (int i = 0; i < cur.Length; i++)
        {
            ...
            if (ch)
            {
                ...
                GUI.Label(new Rect(cxp, vy - vh * ease, chW + 4f * s, vh), prv[i].ToString(), _chipValueStyle);
                GUI.color = new Color(valCol.r, valCol.g, valCol.b, a * ease);
                GUI.Label(new Rect(cxp, vy + vh * (1f - ease), chW + 4f * s, vh), nc.ToString(), _chipValueStyle);
            }
            else
            {
                GUI.color = valCol;
                GUI.Label(new Rect(cxp, vy, chW + 4f * s, vh), nc.ToString(), _chipValueStyle);
            }
        }
        GUI.color = new Color(1f, 1f, 1f, a);
    }
    ...
    private static string PingText()
    {
        try
        {
            int ping = AmongUsClient.Instance != null ? ((InnerNet.InnerNetClient)AmongUsClient.Instance).Ping : -1;
    ...
    private void EnsureStyles()
    {
        if (_stylesReady && _panelTex != null && _btnTex != null && _chipTex != null && _glowTex != null
            && _glossTex != null && _shadowTex != null && _lineTex != null && _moteTex != null && _panelGradTex != null)
            return;

        _stylesReady = false;
        try
        {
            if (_panelTex == null) _panelTex = RoundedTex(new Color(0.03f, 0.035f, 0.05f, 0.98f), 20);
            if (_panelGradTex == null) _panelGradTex = TwoToneVTex(new Color(0.11f, 0.14f, 0.20f, 1f), new Color(0f, 0f, 0f, 0f));
            if (_btnTex == null) _btnTex = RoundedTex(new Color(1f, 1f, 1f, 1f), 16);
            if (_chipTex == null) _chipTex = RoundedTex(new Color(0.08f, 0.09f, 0.12f, 0.96f), 14);
            ...
            _chipLabelStyle = new GUIStyle { alignment = TextAnchor.MiddleLeft, fontSize = 11, fontStyle = FontStyle.Bold };
            _chipLabelStyle.normal.textColor = Color.white;

            _chipValueStyle = new GUIStyle { alignment = TextAnchor.MiddleLeft, fontSize = 16, fontStyle = FontStyle.Bold };
